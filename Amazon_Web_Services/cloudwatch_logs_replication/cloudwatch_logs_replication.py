...
def pull_metrics(newsession_id, newsession_key, newsession_token, newsession_arn, metrics):
    ...
    response = logs.describe_log_streams(logGroupName='object-technica-cloudtrail')
    all_streams += response['logStreams']
    stream_names = [stream['logStreamName'] for stream in all_streams]

    for stream in stream_names:
        events = list()
        metric['logGroupName'] = 'object-technica-cloudtrail'
        metric['logStreamName'] = '{}'.format(stream)

        response = logs.get_log_events(
            logGroupName = 'object-technica-cloudtrail',
            logStreamName = '{}'.format(stream),
            startTime = int((datetime.datetime.now() - timedelta(minutes=15)).timestamp()),
            # endTime = int((datetime.datetime.now() - timedelta(minutes=15)).timestamp())
        )
